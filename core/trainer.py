    ...
    def _train_epoch(self, pbar):
        ...
        for frames, masks, corrupts, _ in self.train_loader:
            ...
            if torch.sum(masks) == 0:
                masks[:, 0, ...] = 1
            # Training trick 2: If all elements in masks are 0, let this masks equal to the last non-zero masks
            # This trick improves the difficulty in the training process which could be more reasonable, not sure yet.
            # if torch.sum(masks) == 0:
            #     masks = masks + self.last_masks
