...
def init_alexnet():
    alexnet_model = lpips.LPIPS(net='alex')
    return alexnet_model

# define a function to calculate the LPIPS score between two images, with a defult model AlexNet
def calculate_lpips(img1, img2, alexnet_model):
    """Calculate LPIPS (Learned Perceptual Image Patch Similarity).
    Ref: https://arxiv.org/abs/1801.03924
    Args:
        img1 (ndarray): Images with range [0, 255].
        img2 (ndarray): Images with range [0, 255].
        model (str): Model name. Default: 'alex'.
    Returns:
        float: lpips result.
    """
    assert img1.shape == img2.shape, \
    ...
    return alexnet_model(img1, img2).item()
