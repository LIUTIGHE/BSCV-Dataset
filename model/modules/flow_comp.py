...
def edge_aware_smoothness_loss(flow, image):
    # gradient of flow
    fx = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).view(1, 1, 3, 3).cuda()
    fy = torch.Tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]]).view(1, 1, 3, 3).cuda()
    u = flow[:, 0:1, :, :]
    v = flow[:, 1:2, :, :]
    fx_u = F.conv2d(u, fx, padding=1)
    fy_u = F.conv2d(u, fy, padding=1)
    fx_v = F.conv2d(v, fx, padding=1)
    fy_v = F.conv2d(v, fy, padding=1)

    # weights
    diff = torch.abs(image[:, :, :-1, :-1] - image[:, :, :-1, 1:]) + torch.abs(image[:, :, :-1, :-1] - image[:, :, 1:, :-1])
    diff = torch.mean(diff, dim=1, keepdim=True)
    diff = diff / torch.mean(diff)

    # loss
    eps = 1e-6
    smoothness_loss = torch.nn.SmoothL1Loss()(fx_u * diff, torch.zeros_like(fx_u)) \
                      + torch.nn.SmoothL1Loss()(fy_u * diff, torch.zeros_like(fy_u)) \
                      + torch.nn.SmoothL1Loss()(fx_v * diff, torch.zeros_like(fx_v)) \
                      + torch.nn.SmoothL1Loss()(fy_v * diff, torch.zeros_like(fy_v))
    return smoothness_loss


class FlowCompletionLoss(nn.Module):
    """Flow completion loss with object integrity alignment."""
    def __init__(self):
        super().__init__()
        ...
        # self.smoothness_criterion = edge_aware_smoothness_loss
    ...
    def forward(self, pred_flows, gt_local_frames):
        ...
        # # calculate loss for edge-aware smoothness
        # forward_smoothness_loss = self.smoothness_criterion(
        #     pred_flows[0].view(-1, 2, h // 4, w // 4), gt_local_frames.view(-1, 3, h // 4, w // 4))
        # backward_smoothness_loss = self.smoothness_criterion(
        #     pred_flows[1].view(-1, 2, h // 4, w // 4), gt_local_frames.view(-1, 3, h // 4, w // 4))
...
def flow_to_image(flow_uv, clip_flow=None, convert_to_bgr=False):
    ...
    u = flow_uv[:, :, 0].detach().cpu().numpy()
    v = flow_uv[:, :, 1].detach().cpu().numpy()
    rad = np.sqrt(np.square(u) + np.square(v))
    rad_max = np.max(rad)
    epsilon = 1e-5
    u = u / (rad_max + epsilon)
